...
st.markdown(
    ...
    <div class="clinical-header">
      ...
      <h1 style="margin:.1rem 0 .2rem 0;color:#0e1f35;">Confirm or Override Final ESI</h1>
      <div style="color:#526070;">Structured model output, clinician judgment, and audit trail in one workflow.</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
if not result:
    st.info("No assessment selected for clinician review.")
    st.caption("Complete New Assessment first, then return here to accept or override the final ESI.")
...
if not assessment_id:
    st.error("This result does not include an assessment ID, so it cannot be saved for review.")
    st.stop()
...
st.markdown('<div class="section-title">Clinician Decision</div>', unsafe_allow_html=True)
st.info(
